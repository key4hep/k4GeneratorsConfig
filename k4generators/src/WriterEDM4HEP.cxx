...
void WriterEDM4HEP::write_event(const GenEvent &evt)
{
    ...
    if (pi) {
        ...
        if ( !status )
        {
            HEPMC3_WARNING("WriterEDM4HEP::write_event: problem serializing GenPdfInfo attribute")
        } else {
...
void WriterEDM4HEP::allocate_buffer()
{
    if ( m_buffer ) return;
    while ( m_buffer == nullptr && m_buffer_size >= 512 ) {
        try {
            ...
            delete[] m_buffer;
            m_buffer_size /= 2;
            HEPMC3_WARNING("WriterEDM4HEP::allocate_buffer:" << e.what() << " buffer size too large. Dividing by 2. New size: " << m_buffer_size)
        }
    }

    if ( !m_buffer )
    {
        HEPMC3_ERROR("WriterEDM4HEP::allocate_buffer: could not allocate buffer!")
        return;
    }
...
std::string WriterEDM4HEP::escape(const std::string& s) const
{
    std::string ret;
...
inline void WriterEDM4HEP::flush()
{
...
inline void WriterEDM4HEP::forced_flush()
{
    std::ptrdiff_t length = m_cursor - m_buffer;
    m_stream->write(m_buffer, length);
    m_cursor = m_buffer;
...
void WriterEDM4HEP::write_particle(const ConstGenParticlePtr& p, int /*second_field*/)
{
    flush();
...
inline void WriterEDM4HEP::write_string(const std::string &str)
{
...
bool WriterEDM4HEP::failed() { return (bool)m_file.rdstate(); }

void WriterEDM4HEP::set_precision(const int& prec ) {
    if (prec < 2 || prec > 24) return;
    m_precision = prec;
}

int WriterEDM4HEP::precision() const {
    return m_precision;
}

void WriterEDM4HEP::set_buffer_size(const size_t& size ) {
    if (m_buffer) return;
    if (size < 1024) return;
    m_buffer_size = size;
}
